...
resource "aws_subnet" "public" {
  ...
  tags = merge(var.tags, {
    Name = "${var.name_prefix}-${var.environment}-public-subnet-${count.index + 1}"
    Type = "Public"
  })
}
...
resource "aws_subnet" "private" {
  ...
  tags = merge(var.tags, {
    Name = "${var.name_prefix}-${var.environment}-private-subnet-${count.index + 1}"
    Type = "Private"
  })
}
...
resource "aws_subnet" "database" {
  ...
  vpc_id            = aws_vpc.main.id
  cidr_block        = cidrsubnet(var.cidr_block, 8, count.index + 200)
  availability_zone = var.availability_zones[count.index]

  tags = merge(var.tags, {
    Name = "${var.name_prefix}-${var.environment}-database-subnet-${count.index + 1}"
    Type = "Database"
  })
}
...
resource "aws_iam_role" "flow_log" {
  name = "${var.name_prefix}-${var.environment}-flow-log-role"
...
resource "aws_iam_role_policy" "flow_log" {
  name = "${var.name_prefix}-${var.environment}-flow-log-policy"
